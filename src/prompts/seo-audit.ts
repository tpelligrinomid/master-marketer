...
function formatKeywordData(intel: SeoIntelligencePackage): string {
  ...
  parts.push(`NOTE: The API returns up to 500 top-ranked keywords per domain. This is a CAPPED SAMPLE, not the total keyword footprint. A domain showing 500 keywords likely ranks for many more. Do NOT report "500 total ranked keywords" as if it's the complete picture — say "500+ ranked keywords (API sample)" or use Keywords Everywhere domain traffic data for total estimates. Similarly, do NOT calculate top-10 ratios from this capped data — if all 500 returned keywords are in the top 10, that just means the API returned the top-performing keywords, not that the domain has a "perfect ranking ratio."\n`);

  const clientKw = intel.client.ranked_keywords;
  if (clientKw?.length) {
    parts.push(`## Client Ranked Keywords (top 200 of ${clientKw.length} returned — API cap is 500)`);
  ...
  for (const comp of intel.competitors) {
    if (comp.ranked_keywords?.length) {
      parts.push(`\n## ${comp.company_name} (${comp.domain}) — Top 50 of ${comp.ranked_keywords.length} returned (API cap is 500)`);
...
function formatCompetitorSeoData(intel: SeoIntelligencePackage): string {
  ...
  parts.push(`NOTE: Keyword counts below are capped at 500 per domain (API limit). Use Keywords Everywhere domain traffic data (if available below) for total organic keyword estimates and traffic. Do NOT treat 500 as the actual total keyword count.\n`);
